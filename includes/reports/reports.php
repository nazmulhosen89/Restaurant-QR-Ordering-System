...
$is_pro = function_exists( 'qrrs_is_pro' ) ? qrrs_is_pro() : true;
$default_report_section = $is_pro ? 'overview' : 'sales';
$pro_only_sections = [ 'overview', 'orders', 'items', 'kitchen', 'staff', 'payment', 'tax' ];
?>
...
<div class="qrrs-reports-wrapper">
    ...
    <div class="report-nav-grid">
        <div class="report-card <?php echo $default_report_section === 'overview' ? 'active' : ''; ?> <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="overview">
            <div class="card-icon">📊</div>
            <div class="card-info"><h3>Overview <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Summary of all</p></div>
        </div>
        <div class="report-card <?php echo $default_report_section === 'sales' ? 'active' : ''; ?>" data-target="sales">
            <div class="card-icon">💰</div>
            <div class="card-info"><h3>Sales Report</h3><p><?php echo $is_pro ? 'Income & Revenue' : 'Last 30 days'; ?></p></div>
        </div>
        <div class="report-card <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="orders">
            <div class="card-icon">🛒</div>
            <div class="card-info"><h3>Order Report <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Order Statistics</p></div>
        </div>
        <!-- <div class="report-card" data-target="items">
            <div class="card-icon">🍔</div>
            <div class="card-info"><h3>Item Performance</h3><p>Best Sellers</p></div>
        </div> -->
        <div class="report-card <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="kitchen">
            <div class="card-icon">🍳</div>
            <div class="card-info"><h3>Kitchen/KOT <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Prep Time & Efficiency</p></div>
        </div>
        <div class="report-card <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="staff">
            <div class="card-icon">👥</div>
            <div class="card-info"><h3>Staff Report <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Performance tracking</p></div>
        </div>
        <div class="report-card <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="payment">
            <div class="card-icon">💳</div>
            <div class="card-info"><h3>Payment Report <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Method Breakdown</p></div>
        </div>
        <div class="report-card <?php echo ! $is_pro ? 'qrrs-pro-locked' : ''; ?>" data-target="tax">
            <div class="card-icon">📜</div>
            <div class="card-info"><h3>VAT/Service <?php echo ! $is_pro ? '<span class="qrrs-pro-badge">Pro</span>' : ''; ?></h3><p>Tax & Charges</p></div>
        </div>
    </div>
    ...
    <div class="report-content-area">
        ...
        foreach ($sections as $section) {
            $class = ($section === $default_report_section) ? 'active' : '';
            echo '<div id="section-' . $section . '" class="report-tab-content ' . $class . '">';

            if ( ! $is_pro && in_array( $section, $pro_only_sections, true ) ) {
                qrrs_render_upgrade_notice( ucfirst( $section ) . ' Report' );
            } else {
